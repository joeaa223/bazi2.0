...
function calculateRelation(zhuObject: { [zhu in '年' | '月' | '日' | '时']: ZhuData }): Relation {
  ...
  for (const zhu1 in zhuObject) {
    const data1 = zhuObject[zhu1 as keyof typeof zhuObject];
    const seen_combinations = new Set<string>();

    for (const zhu2 in zhuObject) {
      if (zhu1 === zhu2) continue;
      const data2 = zhuObject[zhu2 as keyof typeof zhuObject];
      ...
      for (const zhu3 in zhuObject) {
        ...
        const data3 = zhuObject[zhu3 as keyof typeof zhuObject];
...
function getGanChong(gan1: string, gan2: string) {
  ...
  if (GAN_CHONG[joined as keyof typeof GAN_CHONG]) {
    return GAN_CHONG[joined as keyof typeof GAN_CHONG];
  }
}
...
function getGanHe(gan1: string, gan2: string) {
  ...
  if (GAN_WUHE[gan_str as keyof typeof GAN_WUHE]) {
    return GAN_WUHE[gan_str as keyof typeof GAN_WUHE];
  }
}
...
function getZhiChong(zhi1: string, zhi2: string) {
  ...
  if (ZHI_CHONG[zhi_str as keyof typeof ZHI_CHONG]) {
    return ZHI_CHONG[zhi_str as keyof typeof ZHI_CHONG];
  }
}
...
function getZhiHai(zhi1: string, zhi2: string) {
  const zhi_str = zhi1 + zhi2;
  if (HAI_CHONG[zhi_str as keyof typeof HAI_CHONG]) {
    return HAI_CHONG[zhi_str as keyof typeof HAI_CHONG];
  }
}
...
function getZhiAnHe(zhi1: string, zhi2: string) {
  const zhi_str = zhi1 + zhi2;
  if (ZHI_ANHE[zhi_str as keyof typeof ZHI_ANHE]) {
    return ZHI_ANHE[zhi_str as keyof typeof ZHI_ANHE];
  }
}
...
function getZhiHe(zhi1: string, zhi2: string) {
  const zhi_str = zhi1 + zhi2;
  if (ZHI_LIUHE[zhi_str as keyof typeof ZHI_LIUHE]) {
    return ZHI_LIUHE[zhi_str as keyof typeof ZHI_LIUHE];
  }
}
...
function getZhiXing(zhi1: string, zhi2: string) {
  const zhi_str = zhi1 + zhi2;
  if (ZHI_XING[zhi_str as keyof typeof ZHI_XING]) {
    return ZHI_XING[zhi_str as keyof typeof ZHI_XING];
  }
}
